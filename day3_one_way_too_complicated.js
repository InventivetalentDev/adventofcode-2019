...
function findGridSize(inp) {
    ...
    for (let i = 0; i < inp.length; i++) {
        let s = inp[i];
        let d = s.substring(0, 1);
        let c = parseInt(s.substring(1));
    ...
    let xma = Math.max(xp, xn);
    let yma = Math.max(yp, yn);
    let xmi = Math.min(xp, xn);
    let ymi = Math.min(yp, yn);

    let xs = Math.abs(xmax) + Math.abs(xmin);
    let ys = Math.abs(ymax) + Math.abs(ymin);

    // let xdiff = Math.max(Math.abs(xmax), Math.abs(xmin)) - Math.min(Math.abs(xmax), Math.abs(xmin));
    // let ydiff = Math.max(Math.abs(ymax), Math.abs(ymin)) - Math.min(Math.abs(ymax), Math.abs(ymin));
    ...
    return [xs, ys, /*Math.abs(xp - xn), Math.abs(yp - yn)*/xs - xdiff, ys - ydiff];
}

function drawGridLine(grid, inp, start) {
    let pos = start;
    console.log(pos);
    let x = start[0];
    let y = start[1];
    ...
    for (let i = 0; i < inp.length; i++) {
        ...
        pos = drawLine(grid, pos, d, c);
    }

    set(grid, x, y, 3);
}

function drawLine(grid, pos, dir, steps) {
    let dx = 0;
    let dy = 0;
    if (dir === "R") {
        dx = 1;
    ...
    for (let i = 1; i <= steps; i++) {
        set(grid, pos[0] + i * dx, pos[1] + i * dy, 1);
    }

    return [pos[0] + steps * dx, pos[1] + steps * dy];
}
...
function processInput(inputA, inputB) {
    let inpA = typeof inputA === "string" ? inputA.split(",") : inputA;
    let inpB = typeof inputB === "string" ? inputB.split(",") : inputB;


    let gridSizeA = findGridSize(inpA);
    console.log("Grid Size A: " + gridSizeA);
    let gridSizeB = findGridSize(inpB);
    console.log("Grid Size B: " + gridSizeB);

    let gridSize = [Math.max(gridSizeA[0], gridSizeB[0]), Math.max(gridSizeA[1], gridSizeB[1]),
        Math.min(gridSizeA[2], gridSizeB[2]), Math.floor(Math.min(gridSizeA[3], gridSizeB[3]))];
    console.log("Total Grid Size: " + gridSize);
    ...
    drawGridLine(grid, inpA, [gridSize[2], gridSize[3]]);
    // printGrid(grid, gridSize);
    drawGridLine(grid, inpB, [gridSize[2], gridSize[3]]);
    ...
    for (let x = 0; x < gridSize[0]; x++) {
        for (let y = 0; y < gridSize[1]; y++) {
            let v = get(grid, x, y);
            if (v === 2) {
                let d = manhattanDistance([gridSize[2], gridSize[3]], [x, y]);
                console.log("Distance " + [gridSize[2], gridSize[3]] + " -> " + [x, y] + " = " + d);
...
function set(grid, x, y, v) {
    if (x > grid.length) {
        console.warn("x > grid.length (" + x + ")")
    } else if (y > grid[x].length) {
        console.warn("y > grid[x].length (" + y + ")");
    }
    grid[x][y] += v;
}

function setRaw(grid, x, y, v) {
    grid[x][y] = v;
}


// Examples
// console.assert(processInput("R8,U5,L5,D3", "U7,R6,D4,L4") === 6);
console.assert(processInput("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83")===159);
// console.assert(processInput("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7") === 135);

// processInput(inputA, inputB);
